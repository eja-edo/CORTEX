"""Get today's ranked actions tool — the agent's window onto `TodayService`
(Milestone 3.1 M4).
...
async def get_today_handler(args: dict, ctx: ToolContext) -> dict:
    ...
    async with ctx.async_db() as db:
        today = await TodayService(db).get_today(ctx.user_id)
        ...
        return {
            ...
            "needs_confirmation": [
                ...
                for c in today.needs_confirmation
            ],
            "success": True,
        }
