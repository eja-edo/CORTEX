...
# Postgres only grows a native uuidv7() in version 18; we run pg16
# (infrastructure/docker-compose.yml), so the generator lives here as plpgsql.
# It is created in the very first migration because later migrations attach it
# as a column DEFAULT, and Postgres resolves the function at CREATE TABLE time.
UUID_GENERATE_V7 = """
...
DECLARE
    ...
    uuid_bytes bytea;
BEGIN
    -- 48-bit big-endian millisecond timestamp: drop the top 2 bytes of int8send.
    unix_ts_ms := substring(
        int8send(floor(extract(epoch FROM clock_timestamp()) * 1000)::bigint) FROM 3
    );
    -- Start from a v4 for its 122 random bits, then stamp the v7 layout over it.
