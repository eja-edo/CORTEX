...
Standardized from stt_service - single source of truth for all services.
"""
...
class RedisStreamService(Generic[T]):
    ...
    def __init__(
        ...
        # Use ocr_service settings
        self._config = self._create_redis_config()
    ...
    def _create_redis_config(self):
        """Create RedisConfig from ocr_service settings."""
        from dataclasses import dataclass
        ...
        # Parse redis_url from ocr_settings
        redis_url = ocr_settings.redis_url
        host = "localhost"
        port = 6379
        db = 0
        password = ""
