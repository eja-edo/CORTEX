...
| 3.3 | Chỉ có `is_user_busy` (một thời điểm). **Chưa có `find_free_slots`** (dải thời gian) → câu "bạn có 4 tiếng trống" ở màn Hôm nay chưa làm được |
...
| 3.3 | `find_free_slots(user_id, start, end)` | Mở rộng `availability.py`. Mở khoá câu "bạn có 4 tiếng trống" ở màn Hôm nay |
