...
Runs against the real dev Postgres, reusing the seeded user/workspace (same
...
@pytest.mark.asyncio
async def test_build_context_includes_real_workspace(async_db):
    service = ContextService(async_db)
    context = await service.build_context(user_id=TEST_USER_ID, workspace_id=TEST_WORKSPACE_ID)

    assert context.workspace is not None
    assert context.workspace.workspace_id == TEST_WORKSPACE_ID
    assert context.workspace.role == "owner"
    assert context.workspace.member_count >= 1


@pytest.mark.asyncio
async def test_build_context_workspace_none_when_not_provided(async_db):
    service = ContextService(async_db)
    context = await service.build_context(user_id=TEST_USER_ID)
...
@pytest.mark.asyncio
async def test_build_context_workspace_none_for_non_member(async_db):
    service = ContextService(async_db)
    context = await service.build_context(user_id=uuid4(), workspace_id=TEST_WORKSPACE_ID)
    assert context.workspace is None


@pytest.mark.asyncio
async def test_build_context_workspace_none_for_unknown_workspace_id(async_db):
    service = ContextService(async_db)
    context = await service.build_context(user_id=TEST_USER_ID, workspace_id=uuid4())
    assert context.workspace is None
...
async def test_build_context_includes_recent_note(async_db):
    note = Note(user_id=TEST_USER_ID, workspace_id=TEST_WORKSPACE_ID, title="Context Test Note", content="x")
    async_db.add(note)
...
async def test_build_context_excludes_deleted_notes(async_db):
    note = Note(user_id=TEST_USER_ID, workspace_id=TEST_WORKSPACE_ID, title="Deleted Note", content="x", is_deleted=True)
    async_db.add(note)
    await async_db.commit()
    try:
        service = ContextService(async_db)
...
async def test_build_context_filters_notes_for_schedule_intent(async_db, sync_db):
    now = datetime.now(timezone.utc)
    note = Note(user_id=TEST_USER_ID, workspace_id=TEST_WORKSPACE_ID, title="Some Note", content="x")
    async_db.add(note)
    await async_db.commit()
    schedule = Schedule(
...
async def test_build_system_prompt_appends_context_string(async_db):
    ...
    try:
        ...
        assert "Current workspace: Test" in prompt
    finally:
...
@pytest.mark.asyncio
async def test_agent_service_build_context_string_helper(async_db):
    ...
    result = await agent_service._build_context_string(TEST_WORKSPACE_ID, uuid4(), None)
    assert result is None or "Current workspace" in result


@pytest.mark.asyncio
async def test_agent_service_build_context_string_never_raises_on_bad_workspace(async_db):
    """A workspace_id that isn't a real UUID-shaped value some caller might
